...
function theGame($,phys,GameFramework, Box2D, drawutils, mathutils) {
    ...
    game.setSpawnWorldCallback( function () {
        ...
        this.task.numRobotsCollected = 0;
        this.task.numRobotsDied = 0;
        this.task.numBlocksTotal = 5;
        var i;
        var body;
        ...
        contactListener.BeginContact = function(contact, manifold) {
                        ...
                        contact.m_fixtureB.m_body.m_userData === 'robot') 
           {
                ...
                //contact.m_fixtureA.m_body.m_userData = 'workpiece';
    ...
    game.setDrawCallback( function () {
        ...
        // draw goal zone
        var collectionProgress = this.task.numRobotsCollected / this.task.numRobots;
            ...
            for(var f = b.GetFixtureList(); f; f = f.GetNext()) {
                if (bType === 'goal') {
                    ...
                    drawutils.drawRect(30*pos.x, 30*pos.y, 30*X, 30 * Y,
                                ...
                                var angle2 = 90* Math.random();

                this.impulseV.x = 50* Math.cos(angle2);
                this.impulseV.y = 50* Math.sin(angle2);
    ...
    game.setOverviewCallback( function() {
        ...
        if(this.mobileUserAgent){
            drawutils.drawText(300,560, 'use touchscreen to bring 90% of humans home', 1.5, color, color);
        } else {
            drawutils.drawText(300,560, 'use your mouse to bring 90% of humans home', 1.5, color, color);
        }
    ...
    game.setUpdateCallback( function (dt, inputs) {
        ...
        this.task.robots.forEach(function(r) {
             ...
             if( r.GetUserData() === 'robot')
            {
                ...
                this.task.numRobotsCollected ++;
                }
                r.atGoal = true;
                r.SetUserData('empty');
            }
            else if(r.GetUserData()=== 'contact')
            {
        ...
        if (this.controllerActive) {
            ...
            if (this.task.mode === 'global' ) {
                ...
                this.task.robots.forEach( function(r) { 
                    ...
                    var power = 10;
                    this.impulseV.x = power*dx/mag*forceM || 0;
                    this.impulseV.y = power*dy/mag*forceM || 0;
                    if (this.task.mode === 'repulsive') {
                        this.impulseV.x = -power*dx/mag*forceM || 0;
                        this.impulseV.y = -power*dy/mag*forceM || 0;                    
                    } else {
    ...
    game.setWinTestCallback( function() {
        ...
        return (this.task.numRobotsCollected / this.task.numRobots ) >= 0.9;
    });

    game.setLoseTestCallback( function() {
        // if players lose 10% of the robots, they lose
    ...
    game.setLostCallback( function() {
        ...
        var color = 'red';
        drawutils.drawText(300,250, 'You lost. You were able to keep humans ');
    ...
    game.setWonCallback( function() {
       ...
       // nothing to do on lose.
        drawutils.drawRect(300,300, 590,590, 'rgba(200, 200, 200, 0.5)');
        var color = 'green';
        drawutils.drawText(300,250, 'You won! You were able to keep humans ', 2, color,color);
        drawutils.drawText(300,300,'for '+ (this._timeElapsed/1000).toFixed(2) +' seconds!', 2, color, color);
        drawutils.drawText(300,350, 'Loading results page...', 2, color, color);       
    });
