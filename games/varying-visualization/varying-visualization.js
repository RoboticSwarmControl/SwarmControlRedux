...
game.setInitCallback( function() {
    ...
    _.each(this.task.modes, function (m) {
        var curMode = m;
        if( curMode == "mean & variance") {
            curMode = "mean±var";
        }
...
game.setDrawCallback( function() {
    ...
    switch (this.task.mode) {
        case "full-state": for(var i = 0; i < this.task.numRobots; ++i) {
...
game.setUpdateCallback( function (dt, inputs) {
    ...
    if (this.keyDown) {
        this.impulseV.y -= this.task.impulse * Math.min( 1, .001 * ( timeInImpulse/maxImpTime));
    }
    if (this.keyUp) {
        this.impulseV.y += this.task.impulse * Math.min( 1, .001 * ( timeInImpulse/maxImpTime));   
    }
    if (this.keyLeft) {
        this.impulseV.x -= this.task.impulse * Math.min( 1, .001 * ( timeInImpulse/maxImpTime));
    }
    if (this.keyRight) {
        this.impulseV.x += this.task.impulse * Math.min( 1, .001 * ( timeInImpulse/maxImpTime));
    }
